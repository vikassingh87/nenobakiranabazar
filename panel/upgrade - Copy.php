...
if($flag==2){
		...
		// third id update
		 $third=mysqli_query($conn,"SELECT * FROM `tree` where ISNULL(`left`) or ISNULL(`right`)  order by id asc limit 0,1");
						$third_fetch=mysqli_fetch_assoc($third);
						$third_left=$third_fetch['left'];
						$third_right=$third_fetch['right'];
						 $third_id=$third_fetch['id'];
						 $third_uname=$third_fetch['uname'];
						// $find=mysqli_query($conn,"SELECT * FROM `tree` where `id`='$third_id'");
						// $find_fetch=mysqli_fetch_assoc($find);
						//  $third_uname=$find_fetch['uname'];
						if($third_left=='') 
						{
							
						$third_up=mysqli_query($conn,"update `tree` set `left`='$email' ,`super_id`='$userid' where id='$third_id'");
						echo mysqli_error($conn);
						// $next_under_userid = getUnderId($third_id);
						// $temp_side = getUnderIdPlace($third_id);
						// $temp_side_count = 'leftcount';
						// $third_id = $next_under_userid;
						...
						elseif($third_right=='') 
						{
						$third_up=mysqli_query($conn,"update `tree` set `right`='$email' ,`super_id`='$userid' where id='$third_id'");
						echo mysqli_error($conn);
						// $next_under_userid = getUnderId($third_id);
						// $temp_side = getUnderIdPlace($third_id);
						// $temp_side_count = 'rightcount';
						// $third_id = $next_under_userid;
						}
						else
						{
