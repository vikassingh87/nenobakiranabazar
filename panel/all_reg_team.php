                      ...
                      if(mysqli_affected_rows($conn)>0){
                       while($data5 = mysqli_fetch_assoc($query5)){
                        ...
                        if(mysqli_affected_rows($conn)>0){
                         ...
                         $sql7 = "SELECT * FROM user WHERE user_id!= '$data6[sponsor_id]' AND sponsor_id='$data6[user_id]' AND status='0' ";
                          $query7 = mysqli_query($conn,$sql7) or die(mysqli_error($conn));
                          if(mysqli_affected_rows($conn)>0){
                           while($data7 = mysqli_fetch_assoc($query7)){
                            $team_array[] = $data7;

                            $sql8 = "SELECT * FROM user WHERE user_id!= '$data7[sponsor_id]' AND sponsor_id='$data7[user_id]' and status='0'";
                            $query8 = mysqli_query($conn,$sql8) or die(mysqli_error($conn));
                            if(mysqli_affected_rows($conn)>0){
                             while($data8 = mysqli_fetch_assoc($query8)){
                              $team_array[] = $data8;

                              $sql9 = "SELECT * FROM user WHERE user_id!= '$data8[sponsor_id]' AND sponsor_id='$data8[user_id]' and status='0'";
                              $query9 = mysqli_query($conn,$sql9) or die(mysqli_error($conn));
                              if(mysqli_affected_rows($conn)>0){
                               while($data9 = mysqli_fetch_assoc($query9)){
                                $team_array[] = $data9;

                                $sql10 = "SELECT * FROM user WHERE user_id!= '$data9[sponsor_id]' AND sponsor_id='$data9[user_id]' and status='0'";
